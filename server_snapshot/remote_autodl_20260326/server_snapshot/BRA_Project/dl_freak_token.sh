...
LOG="/root/autodl-tmp/BRA_Project/logs/freak_token.log"
echo "[$(date -Iseconds)] FREAK test-00000 START (official HF + HF_TOKEN)" | tee "$LOG"
...
for attempt in range(1, 5):
    try:
        path = hf_hub_download(
            repo_id="hansQAQ/FREAK", filename="data/test-00000-of-00005.parquet",
...
sys.exit(1)
PY
echo "[$(date -Iseconds)] FREAK END" | tee -a "$LOG"
