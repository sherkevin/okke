...
class EntropyMarginTrigger:
    ...
    def decide(
        ...
        if context and context.get("decision_mode") == "answer_labels":
            generated_tokens = self._generated_token_count(input_ids, context)
            if generated_tokens > 0:
                return TriggerDecision(False, "after_answer_step", entropy, margin, content_ratio)
            return TriggerDecision(True, "answer_label_step", entropy, margin, content_ratio)
    ...
    def _contains_binary_label(self, tokenizer: Any, top_ids: torch.Tensor) -> bool:
        for token_id in top_ids.tolist():
            token_text = self._decode_candidate(tokenizer, token_id)
            normalized = self._normalize_binary_label(token_text)
            if normalized in self.BINARY_LABELS:
                return True
        return False

    def _normalize_binary_label(self, token_text: str) -> str:
        raw = token_text.strip()
        raw = raw.replace("Ġ", "").replace("▁", "")
        raw = raw.replace("ï¼¡", "A").replace("ï¼¢", "B")
        return normalize_span(raw)

    def _generated_token_count(self, input_ids: torch.LongTensor, context: dict[str, Any]) -> int:
        prompt_token_count = int(context.get("prompt_token_count") or 0)
        current_len = int(input_ids.shape[-1]) if hasattr(input_ids, "shape") else len(input_ids)
        return max(current_len - prompt_token_count, 0)
