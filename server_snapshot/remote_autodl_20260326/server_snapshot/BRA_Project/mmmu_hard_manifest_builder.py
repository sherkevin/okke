...
def parse_args():
    ...
    parser.add_argument("--output", default="/root/autodl-tmp/BRA_Project/datasets/MMMU_hf/mmmu_hard_manifest.json")
    parser.add_argument("--split", default="validation", choices=["validation", "dev", "test"])
    parser.add_argument("--subjects", nargs="*", default=None, help="Optional whitelist of subject directory names.")
    parser.add_argument("--limit-per-subject", type=int, default=None)
    parser.add_argument("--split-tag", default="mmmu_hard")
    parser.add_argument("--difficulty", default="hard", help="Difficulty filter, e.g. hard. Use '*' to disable.")
