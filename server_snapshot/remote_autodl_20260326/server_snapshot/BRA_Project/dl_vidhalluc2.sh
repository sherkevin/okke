...
mkdir -p "$DEST"
echo "[$(date -Iseconds)] chaoyuli/VidHalluc START (token in config)" | tee "$LOG"
...
for attempt in range(1, 6):
    try:
        snapshot_download(repo_id="chaoyuli/VidHalluc", repo_type="dataset",
                          local_dir=dest, max_workers=4)
        print("OK chaoyuli/VidHalluc -> " + dest, flush=True); sys.exit(0)
