...
IMAGE_DIR = PROJECT_ROOT / "data" / "blurred_calibration" / "blur"
...
def infer_dynamic(
    model, processor, image: Image.Image, prompt: str, aosp_handle=None, max_new_tokens: int = 64
) -> dict:
    inputs = processor(
        text=[prompt],
        images=[image],
...
def run_eval(args):
    ...
    images = sorted(list(IMAGE_DIR.glob("*.jpg")))[:args.limit]
    ds = [{"question_id": i, "image_path": str(img)} for i, img in enumerate(images)]
    ...
    for idx, sample in enumerate(pending):
        ...
        if (idx + 1) % 5 == 0 or (idx + 1) == len(pending):
            print(f"[{idx+1}/{len(pending)}] qid={qid} len={result['gen_len']} interv={result['interventions']}")
    ...
    print(f"\n[Done] Avg Gen Length: {total_length/len(pending):.2f}, Avg Interventions: {total_interventions/len(pending):.2f}")
...
def parse_args():
    ...
    p.add_argument("--dynamic_mu", action="store_true", help="Enable dynamic mu based on visual entropy")
    return p.parse_args()
