...
def build_chartqa_prompt(sample: dict) -> str:
    question = sample.get("query", "")
    prompt = f"{question}\nAnswer directly with a number or short phrase."
    return (
...
def infer_chartqa(
    model, processor, image: Image.Image, prompt: str, aosp_handle=None, max_new_tokens: int = 32
) -> dict:
    inputs = processor(
        text=[prompt],
        images=[image],
...
def run_eval(args):
    ...
    for idx, sample in enumerate(pending):
        ...
        try:
            ...
            prompt = build_chartqa_prompt(sample)
            result = infer_chartqa(
        ...
        record = {
            ...
            "ground_truth": sample.get("label", [""])[0] if isinstance(sample.get("label"), list) else sample.get("label", ""),
        ...
        if (idx + 1) % 5 == 0 or (idx + 1) == len(pending):
            print(f"[{idx+1}/{len(pending)}] qid={qid} pred='{result['text']}' gt='{record['ground_truth']}' interv={result['interventions']}")
...
def parse_args():
    ...
    p.add_argument("--dataset_file", type=str, default="/root/autodl-tmp/A-OSP_Project/data/benchmarks/chartqa/chartqa_manifest.jsonl")
