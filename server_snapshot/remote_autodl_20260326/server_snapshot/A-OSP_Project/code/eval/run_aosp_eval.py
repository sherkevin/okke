#!/usr/bin/env python3
"""
A-OSP Evaluation Pipeline — A-OSP Intervention POPE Evaluation
================================================================
Evaluates Qwen2-VL-7B-Instruct WITH A-OSP orthogonal subspace projection
on POPE benchmark. This is the intervention group to compare against the
Base baseline produced by run_base_eval.py.

Key differences from Base:
  1. Loads V_matrix.pt and installs the A-OSP hook via aosp_hook.apply_aosp_hook()
  2. Calls handle.reset() before each model.generate() to clear per-sequence state
  3. Records intervention_count per sample for diagnostics (how often the brake fired)

All engineering guarantees (JSONL Append, VRAM reclamation, AGL tracking) identical.

Usage:
    python run_aosp_eval.py \
        ...
        --tag         aosp_qwen2vl7b_pope_popular
"""
...
# ──────────────────────────────────────────────────────────────────────────
# Inference for a single POPE sample WITH A-OSP intervention
# ──────────────────────────────────────────────────────────────────────────

def infer_single_pope_aosp(
    model,
    processor,
    image: Image.Image,
    question: str,
    ...
    """
    Run one POPE question through Qwen2-VL / Qwen3-VL with A-OSP active.
    Returns (generated_text, generation_length, latency_seconds, intervention_count).
    """
    # Use apply_chat_template for cross-model compatibility
    messages = [{
        "role": "user",
        "content": [
    ...
    n_interventions = handle.intervention_count

    del inputs, output_ids, gen_ids
    return gen_text, int(gen_len), latency, n_interventions
...
def run_aosp_pope_eval(args):
    ...
    for idx, sample in enumerate(pending):
        ...
        try:
            gen_text, gen_len, latency, n_interv = infer_single_pope_aosp(
                model, processor, image, question, handle,
                max_new_tokens=args.max_new_tokens,
            )
        except Exception as exc:
            print(f"[ERROR] qid={qid}: {exc}")
            gen_text, gen_len, latency, n_interv = "error", 0, 0.0, 0
            errors += 1
    ...
    # ── final metrics ──
    print("\n" + "=" * 60)
    print("[Eval] Computing final A-OSP POPE metrics ...")
    all_results = load_jsonl(result_file)
    metrics = compute_pope_metrics(all_results)
    metrics["method"] = "aosp"
    ...
    metrics["total_interventions"] = total_interventions
    metrics["avg_interventions_per_sample"] = round(
        total_interventions / max(len(all_results), 1), 2
    )
    metrics["hyperparams"] = {
        "alpha": args.alpha,
        "mu": args.mu,
...
def parse_args():
    p = argparse.ArgumentParser(description="A-OSP POPE Evaluation")
    p.add_argument(
        "--model_path", type=str,
    ...
    p.add_argument("--tag", type=str, default="aosp_qwen2vl7b_pope_popular")
    p.add_argument("--max_new_tokens", type=int, default=64)
    p.add_argument("--limit", type=int, default=0, help="Max samples to eval (0=all)")
    # A-OSP hyperparameters (paper defaults)
...
if __name__ == "__main__":
    args = parse_args()
    print("=" * 60)
    print("  A-OSP Evaluation Pipeline — POPE with A-OSP Intervention")
