...
def _load_hallusionbench_samples(limit: int):
    import pandas as pd

    df = pd.read_parquet(HALLUSIONBENCH_FILE).head(limit)
...
def _load_mmbench_samples(limit: int):
    import pandas as pd

    df = pd.read_parquet(MMBENCH_FILE).head(limit)
    image_cols = [c for c in df.columns if "image" in c.lower()]
    samples = []
    for _, row in df.iterrows():
        image = _parquet_image(row, image_cols)
...
def _load_mme_samples(limit: int):
    import pandas as pd
...
def run_generic_benchmark(model, processor, method, args, *, records_by_index: dict[int, dict] | None = None, sample_log_path: Path | None = None, persist_callback=None):
    if args.dataset == "hallusionbench":
        samples = _load_hallusionbench_samples(args.mini_test)
        answer_type = "yes_no"
    elif args.dataset == "mmbench":
        samples = _load_mmbench_samples(args.mini_test)
        answer_type = "letter"
    elif args.dataset == "mme":
        samples = _load_mme_samples(args.mini_test)
        answer_type = "yes_no"
    else:
...
def run_pope(model, processor, method, args, *, records_by_index: dict[int, dict] | None = None, sample_log_path: Path | None = None, persist_callback=None):
    ...
    samples = [json.loads(l) for l in open(split_file)][:args.mini_test]
...
def load_karpathy_test_images(limit: int):
...
def run_chair(model, processor, method, args, *, records_by_index: dict[int, dict] | None = None, sample_log_path: Path | None = None, persist_callback=None):
    ...
    images = load_karpathy_test_images(args.mini_test)
