...
def summarize_run_outputs(run_outputs: list[dict]) -> dict:
    ...
    sample_times = [float(item["sample_time_ms"]) for item in run_outputs if item.get("sample_time_ms") is not None]
    generated_tokens = [int(item["generated_tokens"]) for item in run_outputs if item.get("generated_tokens") is not None]
    ...
    return {
        ...
        "mean_sample_time_ms": mean(sample_times) if sample_times else 0.0,
        "mean_generated_tokens": mean(generated_tokens) if generated_tokens else 0.0,
...
def main():
    ...
    quantized_llava = bool(
        args.model == "llava-1.5"
        and (model_config.get("load_8bit", False) or model_config.get("load_4bit", False))
    )
    if not quantized_llava:
    ...
    for batch_id, data in tqdm(enumerate(pope_loader), total=len(pope_loader)):
        ...
        with torch.inference_mode():
            with torch.no_grad():
                ...
                if torch.cuda.is_available():
                    torch.cuda.synchronize(device)
                sample_time_ms = (time.perf_counter() - sample_start) * 1000.0
                pred_list = recorder(out, pred_list)
                for line in out:
                    print(line)
                generated_tokens = None
                if hasattr(model, "llama_tokenizer"):
                    generated_tokens = sum(
                        len(model.llama_tokenizer(text, add_special_tokens=False).input_ids)
                        for text in out
                    )
                run_outputs.append(
                    {
