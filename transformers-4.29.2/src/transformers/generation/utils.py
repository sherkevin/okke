...
class GenerationMixin:
    ...
    def generate(
        ...
        chord_visual_token_weights: Optional[torch.Tensor] = None,
        chord_lambda_cur: Optional[float] = 0.0,
        chord_lambda_fut: Optional[float] = 0.0,
        chord_lambda_txt: Optional[float] = 1.0,
        chord_zero_anchor_penalty: Optional[float] = 0.0,
        chord_future_horizon: Optional[int] = 0,
        chord_future_topk: Optional[int] = 0,
        chord_tau_abort: Optional[float] = 0.0,
        chord_attention_last_n_layers: Optional[int] = 2,
        chord_attention_head_reduce: Optional[str] = "mean",
        chord_diagnostics: Optional[list] = None,
        **kwargs,
    ...
    def chord_beam_search(
        ...
        **model_kwargs,
        ...
        attention_last_n_layers = max(1, int(chord_attention_last_n_layers or 1))
        attention_head_reduce = str(chord_attention_head_reduce or "mean")
        ...
        future_requested = (
            chord_visual_token_weights is not None
            and float(chord_lambda_fut) != 0.0
            and int(chord_future_horizon) > 0
            and int(chord_future_topk) > 0
        )
        future_low_score = -1.0e4
        tau_abort = float(chord_tau_abort if chord_tau_abort is not None else 0.0)
        future_rollout_batch_limit = 8
        future_candidate_chunk_limit = 1

        def _slice_batch_like(value, active_indices: torch.Tensor):
            if isinstance(value, torch.Tensor):
                if value.dim() > 0 and value.shape[0] > int(active_indices.max().item()):
                    return value.index_select(0, active_indices.to(value.device))
                return value
            if isinstance(value, tuple):
                return tuple(_slice_batch_like(item, active_indices) for item in value)
            if isinstance(value, list):
                return [_slice_batch_like(item, active_indices) for item in value]
            if isinstance(value, dict):
                return {key: _slice_batch_like(item, active_indices) for key, item in value.items()}
            return value

        def _concat_batch_like(values):
            first = values[0]
            if isinstance(first, torch.Tensor):
                if first.dim() > 0 and first.shape[0] == input_ids.shape[0]:
                    return torch.cat(values, dim=0)
                return first
            if isinstance(first, tuple):
                return tuple(_concat_batch_like([value[idx] for value in values]) for idx in range(len(first)))
            if isinstance(first, list):
                return [_concat_batch_like([value[idx] for value in values]) for idx in range(len(first))]
            if isinstance(first, dict):
                return {key: _concat_batch_like([value[key] for value in values]) for key in first}
            return first

        def _compute_future_candidate_scores(
            *,
            candidate_tokens: torch.Tensor,
            bootstrap_seed_model_kwargs: dict,
            expanded_token_weights: torch.Tensor | None,
            attn_pos: dict,
            v_anchor_scores: torch.Tensor,
        ) -> tuple[
            torch.Tensor,
            torch.Tensor,
            torch.Tensor,
            ...
            zero_scores = torch.zeros_like(candidate_tokens, dtype=torch.float32)
            zero_counts = torch.zeros_like(candidate_tokens, dtype=torch.float32)
            failed_candidates = [[False for _ in range(candidate_tokens.shape[0])] for _ in range(num_attn_candidates)]
            error_candidates = [[None for _ in range(candidate_tokens.shape[0])] for _ in range(num_attn_candidates)]
            skipped_no_anchor = [[False for _ in range(candidate_tokens.shape[0])] for _ in range(num_attn_candidates)]
            aborted_tau = [[False for _ in range(candidate_tokens.shape[0])] for _ in range(num_attn_candidates)]
            future_enabled = (
                expanded_token_weights is not None
                and float(chord_lambda_fut) != 0.0
                and int(chord_future_horizon) > 0
                and int(chord_future_topk) > 0
            )
            ...
            image_indices = torch.arange(
                ...
                attn_pos["image_end"] + 1,
                device=input_ids.device,
                dtype=torch.long,
            )
            candidate_limit = min(int(chord_future_topk), num_attn_candidates)
            sum_visual_scores = torch.zeros_like(candidate_tokens, dtype=torch.float32)
            sum_text_scores = torch.zeros_like(candidate_tokens, dtype=torch.float32)
            future_scores = torch.zeros_like(candidate_tokens, dtype=torch.float32)
            token_weights = expanded_token_weights.to(dtype=torch.float32)
            active_after_anchor = [0 for _ in range(num_attn_candidates)]
            active_after_tau = [0 for _ in range(num_attn_candidates)]
            ...
            try:
                num_branch_rows = candidate_tokens.shape[0]
                for candidate_start in range(0, candidate_limit, future_candidate_chunk_limit):
                    candidate_stop = min(candidate_start + future_candidate_chunk_limit, candidate_limit)
                    local_count = candidate_stop - candidate_start
                    branch_mask = (v_anchor_scores[:, candidate_start:candidate_stop] > 0.0).transpose(0, 1).contiguous()

                    for local_candidate_id in range(local_count):
                        global_candidate_id = candidate_start + local_candidate_id
                        active_after_anchor[global_candidate_id] = int(branch_mask[local_candidate_id].sum().item())
                        if active_after_anchor[global_candidate_id] == 0:
                            skipped_no_anchor[global_candidate_id] = [True for _ in range(num_branch_rows)]
                        active_after_tau[global_candidate_id] = active_after_anchor[global_candidate_id]

                    if not bool(branch_mask.any().item()):
                        continue

                    flat_branch_indices = torch.nonzero(branch_mask.reshape(-1), as_tuple=False).flatten()
                    active_row_ids = torch.remainder(flat_branch_indices, num_branch_rows)
                    flat_candidate_tokens = (
                        candidate_tokens[:, candidate_start:candidate_stop]
                        .transpose(0, 1)
                        .reshape(-1)
                        .index_select(0, flat_branch_indices.to(candidate_tokens.device))
                        .unsqueeze(-1)
                    )
                    seed_model_kwargs = _slice_batch_like(
                        bootstrap_seed_model_kwargs,
                        active_row_ids.to(input_ids.device),
                    )
                    model_inputs_bootstrap = self.prepare_inputs_for_generation(flat_candidate_tokens, **seed_model_kwargs)
                    outputs_bootstrap = self(
                        **model_inputs_bootstrap,
                        return_dict=True,
                        output_attentions=output_attentions,
                        output_hidden_states=output_hidden_states,
                        last_attention_only=attention_last_n_layers if output_attentions else False,
                    )
                    flat_attn_rows = reduce_attention_maps(
                        outputs_bootstrap.attentions,
                        last_n_layers=attention_last_n_layers,
                        head_reduce=attention_head_reduce,
                    )[:, -1, :].to(torch.float32)
                    flat_token_weights = token_weights.repeat(local_count, 1).index_select(
                        0, flat_branch_indices.to(token_weights.device)
                    )
                    flat_sum_visual, flat_sum_text = summarize_rollout_step_batch(
                        attn_last_rows=flat_attn_rows,
                        prefix_length=input_ids.shape[1] + 1,
                        visual_token_indices=image_indices,
                        image_token_span=(attn_pos["image_start"], attn_pos["image_end"]),
                        token_weights=flat_token_weights,
                    )

                    active_candidate_ids = torch.div(flat_branch_indices, num_branch_rows, rounding_mode="floor")
                    active_abort_mask = torch.zeros_like(flat_sum_visual, dtype=torch.bool)
                    if tau_abort > 0.0:
                        active_abort_mask = compute_visual_anchor_ratio(flat_sum_visual, flat_sum_text) < tau_abort

                    if active_abort_mask.any():
                        abort_candidate_ids = active_candidate_ids[active_abort_mask].tolist()
                        abort_row_ids = active_row_ids[active_abort_mask].tolist()
                        for local_candidate_id, row_idx in zip(abort_candidate_ids, abort_row_ids):
                            global_candidate_id = candidate_start + local_candidate_id
                            aborted_tau[global_candidate_id][row_idx] = True
                            active_after_tau[global_candidate_id] -= 1

                    future_scores_candidate_major = torch.zeros(
                        (local_count, num_branch_rows),
                        dtype=torch.float32,
                        device=future_scores.device,
                    )
                    sum_visual_candidate_major = torch.zeros_like(future_scores_candidate_major)
                    sum_text_candidate_major = torch.zeros_like(future_scores_candidate_major)
                    if active_abort_mask.any():
                        future_scores_candidate_major[
                            active_candidate_ids[active_abort_mask].to(future_scores_candidate_major.device),
                            active_row_ids[active_abort_mask].to(future_scores_candidate_major.device),
                        ] = future_low_score

                    survivor_flat_indices = flat_branch_indices[~active_abort_mask]
                    if survivor_flat_indices.numel() > 0:
                        running_sum_visual = flat_sum_visual[~active_abort_mask].to(torch.float32)
                        running_sum_text = flat_sum_text[~active_abort_mask].to(torch.float32)
                        active_token_weights = flat_token_weights[~active_abort_mask]
                        current_tokens = outputs_bootstrap.logits[:, -1].argmax(dim=-1)[~active_abort_mask].unsqueeze(-1)
                        running_model_kwargs = self._update_model_kwargs_for_generation(
                            outputs_bootstrap,
                            seed_model_kwargs,
                            is_encoder_decoder=self.config.is_encoder_decoder,
                        )
                        running_model_kwargs = _slice_batch_like(
                            running_model_kwargs,
                            torch.nonzero(~active_abort_mask, as_tuple=False).flatten().to(input_ids.device),
                        )

                        for rollout_step in range(1, int(chord_future_horizon)):
                            if current_tokens.shape[0] <= future_rollout_batch_limit:
                                model_inputs_roll = self.prepare_inputs_for_generation(current_tokens, **running_model_kwargs)
                                outputs_roll = self(
                                    **model_inputs_roll,
                                    return_dict=True,
                                    output_attentions=output_attentions,
                                ...
                                attn_roll_rows = reduce_attention_maps(
                                    outputs_roll.attentions,
                                    last_n_layers=attention_last_n_layers,
                                    head_reduce=attention_head_reduce,
                                )[:, -1, :]
                                current_tokens = outputs_roll.logits[:, -1].argmax(dim=-1).unsqueeze(-1)
                                running_model_kwargs = self._update_model_kwargs_for_generation(
                                    outputs_roll,
                                    running_model_kwargs,
                                    is_encoder_decoder=self.config.is_encoder_decoder,
                                ...
                                attn_roll_rows_chunks = []
                                next_tokens_chunks = []
                                model_kwargs_chunks = []
                                for start in range(0, current_tokens.shape[0], future_rollout_batch_limit):
                                    end = min(start + future_rollout_batch_limit, current_tokens.shape[0])
                                    chunk_indices = torch.arange(start, end, device=current_tokens.device, dtype=torch.long)
                                    chunk_tokens = current_tokens[start:end]
                                    chunk_model_kwargs = _slice_batch_like(running_model_kwargs, chunk_indices)
                                    model_inputs_roll = self.prepare_inputs_for_generation(chunk_tokens, **chunk_model_kwargs)
                                    outputs_roll = self(
                                        **model_inputs_roll,
                                        return_dict=True,
                                        output_attentions=output_attentions,
                                    ...
                                    next_tokens_chunks.append(outputs_roll.logits[:, -1].argmax(dim=-1).unsqueeze(-1))
                                    model_kwargs_chunks.append(
                                ...
                                attn_roll_rows = torch.cat(attn_roll_rows_chunks, dim=0)
                                current_tokens = torch.cat(next_tokens_chunks, dim=0)
                                running_model_kwargs = _concat_batch_like(model_kwargs_chunks)
                            step_sum_visual, step_sum_text = summarize_rollout_step_batch(
                                attn_last_rows=attn_roll_rows,
                                prefix_length=input_ids.shape[1] + 1 + rollout_step,
                                visual_token_indices=image_indices,
                                image_token_span=(attn_pos["image_start"], attn_pos["image_end"]),
                                token_weights=active_token_weights,
                            )
                            running_sum_visual += step_sum_visual.to(running_sum_visual.device)
                            running_sum_text += step_sum_text.to(running_sum_text.device)

                        flat_future_scores = future_scores_candidate_major.view(-1)
                        flat_sum_visual_out = sum_visual_candidate_major.view(-1)
                        flat_sum_text_out = sum_text_candidate_major.view(-1)
                        flat_future_scores[survivor_flat_indices.to(flat_future_scores.device)] = running_sum_visual.to(flat_future_scores.device) - float(chord_lambda_txt) * running_sum_text.to(flat_future_scores.device)
                        flat_sum_visual_out[survivor_flat_indices.to(flat_sum_visual_out.device)] = running_sum_visual.to(flat_sum_visual_out.device)
                        flat_sum_text_out[survivor_flat_indices.to(flat_sum_text_out.device)] = running_sum_text.to(flat_sum_text_out.device)

                    future_scores[:, candidate_start:candidate_stop] = future_scores_candidate_major.transpose(0, 1)
                    sum_visual_scores[:, candidate_start:candidate_stop] = sum_visual_candidate_major.transpose(0, 1)
                    sum_text_scores[:, candidate_start:candidate_stop] = sum_text_candidate_major.transpose(0, 1)
            except Exception as exc:
                for candidate_id in range(candidate_limit):
                    active_indices = torch.nonzero(v_anchor_scores[:, candidate_id] > 0.0, as_tuple=False).flatten()
                    if active_indices.numel() == 0:
                        active_indices = torch.arange(candidate_tokens.shape[0], device=input_ids.device)
                    for row_idx in active_indices.tolist():
                        failed_candidates[candidate_id][row_idx] = True
                        error_candidates[candidate_id][row_idx] = str(exc)
        ...
        while True:
            ...
            if not "past_key_values" in model_kwargs.keys():
                attn_previous = reduced_attn.clone() # [batch_size * num_beams, 1, q, kv]
            else:
                ...
                attn_previous = torch.cat(
                    [attn_previous[beam_idx], reduced_attn.data], -2) # [batch_size * num_beams, 1, q, kv]
            ...
            attn_last = []
            future_bootstrap_seed_model_kwargs = None
            for candidate_id in range(num_attn_candidates):
                ...
                if future_bootstrap_seed_model_kwargs is None:
                    future_bootstrap_seed_model_kwargs = model_kwargs.copy()
                    future_bootstrap_seed_model_kwargs = self._update_model_kwargs_for_generation(
                        outputs, future_bootstrap_seed_model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
                    )
                model_kwargs_tmp = future_bootstrap_seed_model_kwargs
                ...
                reduced_candidate_attn = reduce_attention_maps(
                    outputs_tmp.attentions,
                    last_n_layers=attention_last_n_layers,
                    head_reduce=attention_head_reduce,
                )
                bootstrap_attn_rows = reduced_candidate_attn[:, -1, :].to(torch.float32)

                attn_output = bootstrap_attn_rows.unsqueeze(1).unsqueeze(1).data # [batch_size * num_beams, 1, 1, kv+1]
            ...
            expanded_token_weights = None
            if chord_visual_token_weights is not None:
                expanded_token_weights = chord_visual_token_weights
                if expanded_token_weights.dim() == 1:
                    expanded_token_weights = expanded_token_weights.unsqueeze(0)
                if expanded_token_weights.shape[0] == 1 and input_ids.shape[0] > 1:
                    expanded_token_weights = expanded_token_weights.expand(input_ids.shape[0], -1)
                if expanded_token_weights.shape[0] != input_ids.shape[0]:
                    raise ValueError("chord_visual_token_weights batch dimension must match the active beam batch.")
            ...
            (
                ...
                future_sum_text_scores,
                future_failed_candidates,
                future_error_candidates,
                future_skipped_no_anchor,
                future_aborted_tau,
                future_active_after_anchor,
                future_active_after_tau,
            ) = _compute_future_candidate_scores(
                candidate_tokens=candidate_tokens,
                bootstrap_seed_model_kwargs=future_bootstrap_seed_model_kwargs or {},
                expanded_token_weights=expanded_token_weights,
                attn_pos=attn_pos,
            ...
            try:
                ...
                if float(chord_lambda_fut) != 0.0:
                    ...
                    candidate_token_scores_fused = candidate_token_scores_current
                if chord_diagnostics is not None:
                    after_order = torch.argsort(candidate_token_scores_fused, dim=-1, descending=True, stable=True)
                    chord_diagnostics.append(
                        {
                            "step_index": int(cur_len),
                            "rollback_triggered": False,
                            "candidate_token_ids_before": candidate_tokens.detach().cpu().tolist(),
                            "candidate_token_ids_after": candidate_tokens.gather(1, after_order).detach().cpu().tolist(),
                            "v_anchor": v_anchor_scores.detach().cpu().tolist(),
                            "sum_v_tau": future_sum_visual_scores.detach().cpu().tolist(),
                            "sum_t_tau": future_sum_text_scores.detach().cpu().tolist(),
                            "f_future": f_future_scores.detach().cpu().tolist(),
                            "future_failed": future_failed_candidates,
                            "future_errors": future_error_candidates,
                            "future_skipped_no_anchor": future_skipped_no_anchor,
                            "future_aborted_tau": future_aborted_tau,
                            "future_active_after_anchor": future_active_after_anchor,
                            "future_active_after_tau": future_active_after_tau,
                        }
                    )
                next_token_logits.fill_(-999.)
                next_token_logits = next_token_logits.scatter_(-1, candidate_tokens, candidate_token_scores_fused)
