...
def reviewer_system_prompt() -> str:
    return (
        ...
        "Do not be boxed in by the paper's current self-description; if a narrower, different, or more negative interpretation is more defensible, say so plainly."
    )
...
def scientist_system_prompt() -> str:
    return (
        ...
        "Do not chase polish for its own sake; use the review to sharpen the method, protocol, and benchmark."
    )


def build_reviewer_prompt(version: int, paper_text: str) -> str:
    return f"""
你是 ACM MM 会议的的首席主审稿人，请公正严厉刻薄地评价我的论文研究内容，尖锐地指出其中所有的缺点和不足，不要任何讨好我的想法，不要有优化建议，直接给出你的评价和本次 review 的打分，满分是五分：
...
8. 作者当前实验尚未完成，因此请像真正的强审稿人一样重点审查“实验计划是否合理、是否可证伪、是否会被偏置或效率问题击穿”，而不是只因为结果未完成就给出表面化意见。
9. 你不需要接受作者对自己方法的自我定义。如果你认为作者真正站得住的贡献比正文写得更窄，或者根本不是作者声称的那一点，请明确指出。
10. 你可以自由挑战：问题定义、方法身份、与 baseline 的关系、实验合同、系统成本、适用模态范围、失败边界、以及是否存在更合理的 fallback claim。
11. 如果你发现作者对竞争基线的机制描述存在事实性偏差，请直接指出并纠正。
12. 如果你认为某个实验、图表、附录、负控制或失败分析并不是可有可无，而是接受与否的关键门槛，请明确把它提升为必须项。
13. 你的任务不是帮助作者把文章“写圆”，而是尽可能暴露真正决定论文成败的硬约束与硬边界。
...
def build_scientist_prompt(
    ...
    return f"""
你是全球顶级 AI 科学家，是 ACM MM 的资深投稿者和多年审稿人。你需要根据审稿意见对原稿进行针对性精修，但你的首要目标不是把文章修饰得更完整，而是把方法、实验合同、失败边界和 fallback claim 打磨得更真实、更有科研价值。
...
7. 如果 reviewer 提出的更窄、更硬、更有价值的解释比当前正文更好，你可以主动收缩或重组叙事，不必机械维护现有包装。
8. 当前必须保留的核心只有：
...
9. 不要再把 `Pooling Paradox` 当成标题、摘要或引言的主题眼；如果要提到 coarse aggregation，也只能作为内部机制对照，不能再当作领域总批判。
10. `DoLa`、`VCD`、`OPERA` 只能作为竞争基线，不得被错误描述成依赖全局视觉池化的方法。
11. 若视频证据不足，你应主动把视频维持为次主线或 appendix，而不是为了 ACM MM 叙事机械保留。
12. 不要让 OCR-heavy documents、adapter 身份、WordNet 工程细节、EMA 细节喧宾夺主；如果这些内容开始挤掉核心方法命题，你必须主动收缩它们。
13. 如果方法对 OCR token 的有效干预尚未自洽，不要再把 DocVQA / OCR-heavy documents 写成核心动机；可以把它们降级为边界测试或条件性扩展。
...
def build_scientist_compact_prompt(
    ...
    memory_block = scientist_memory.strip() if scientist_memory.strip() else "当前还没有历史科学家记忆。"
    return f"""
你是全球顶级 AI 科学家，是 ACM MM 的资深投稿者和多年审稿人。你现在要对论文进行“精准微调式升级”，而不是整篇推倒重写。你的目标不是追求表面完整度，而是把真正有价值的理论、协议和失败边界打磨出来。
...
8. 如果 reviewer 给出的更窄、更硬、更可证伪的主张比当前包装更好，你应主动收缩或重组叙事，而不是为现有说法做辩护。
9. 当前 benchmark 口径是 image-centric 主合同 + bounded secondary video pilot。视频只有在你能让它服务于“token-local visual evidence 在时序场景也成立”时才应保留为次主线；否则继续降级，不得挤占图像主线。
10. 你必须把实验设计明确组织成三条证据链和五道防线：
...
11. 如果正文开始围绕 OCR paradox、PEFT 身份、WordNet dictionary、EMA kill-switch 等工程边界无限扩写，你必须主动收缩，把它们从主叙事降回 limitation / protocol / appendix，而不是让它们接管论文。
12. 当前新版本必须比旧自动链条更像“真正可投稿的核心版本”：减少摇摆叙事，明确正面方法 claim、主实验表、关键消融、失败案例和可执行协议。
