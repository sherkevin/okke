#!/usr/bin/env python3
"""Sync baseline_sample_preview_runner.py (and its deps) to the remote server and launch it on GPU0."""
...
REMOTE_PREVIEW_LOG_DIR = f"{REMOTE_PROJECT}/logs/baseline_delivery_sample"
...
FILES_TO_SYNC = (
    ...
    "baseline_sample_preview_runner.py",
)
...
def sync_files() -> None:
    run_remote_bash(
        f"set -euo pipefail\nmkdir -p {REMOTE_PROJECT} {REMOTE_PREVIEW_LOG_DIR}\n",
        timeout=120,
        check=True,
    )
...
def build_launch_script(*, log_path: str) -> str:
...
mkdir -p {REMOTE_PREVIEW_LOG_DIR}
nohup {REMOTE_PYTHON} {REMOTE_PROJECT}/baseline_sample_preview_runner.py >> "{log_path}" 2>&1 < /dev/null &
echo "LAUNCH_PID=$!"
echo "LOG={log_path}"
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Sync and launch the sample-preview runner on remote GPU0."
    )
    parser.add_argument("--mode", choices=["sync", "launch"], default="launch",
                        help="'sync' only syncs files; 'launch' syncs then starts the runner.")
    ...
    log_path = f"{REMOTE_PREVIEW_LOG_DIR}/baseline_sample_preview_gpu0_{ts}.log"
    proc = run_remote_bash(
        build_launch_script(log_path=log_path),
        timeout=120,
        check=False,
    )
