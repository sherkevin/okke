...
from chord.anchor_builder import build_visual_token_weights
...
class CHORDTheoryLockTests(unittest.TestCase):
    ...
    def test_query_miss_returns_uniform_visual_weights(self) -> None:
        weights = build_visual_token_weights(
            ...
            relevance=torch.zeros(2, dtype=torch.float32),
            confidence=torch.tensor([0.95, 0.85], dtype=torch.float32),
            alpha_anchor=0.5,
        )
