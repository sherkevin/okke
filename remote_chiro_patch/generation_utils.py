...
from chord.chord_fusion import apply_current_chord_score, fuse_chord_scores
from chord.future_rollout import greedy_future_rollout_from_bootstrap
...
class GenerationMixin:
    ...
    def opera_beam_search(
        ...
        **model_kwargs,
        ...
        def _clone_past_key_values_row(source_past_key_values, row_idx, target_devices=None):
            row_past = []
            for layer_idx, layer_state in enumerate(source_past_key_values):
                row_layer = []
                for tensor_idx, tensor in enumerate(layer_state):
                    row_tensor = tensor[row_idx:row_idx + 1].clone()
                    if target_devices is not None:
                        target_device = target_devices[layer_idx][tensor_idx]
                        if row_tensor.device != target_device:
                            row_tensor = row_tensor.to(target_device)
                    row_layer.append(row_tensor)
                row_past.append(tuple(row_layer))
            return tuple(row_past)

        def _slice_model_kwargs_row(source_model_kwargs, row_idx, target_device=None, include_past=True):
            row_kwargs = {}
            for key, value in source_model_kwargs.items():
                if key == "past_key_values" and value is not None:
                    if not include_past:
                        continue
                    # Preserve the original per-layer device placement of sharded
                    # caches instead of collapsing every layer onto one GPU.
                    row_kwargs[key] = _clone_past_key_values_row(value, row_idx)
                elif isinstance(value, torch.Tensor) and value.dim() > 0 and value.shape[0] == input_ids.shape[0]:
                    row_tensor = value[row_idx:row_idx + 1].clone()
                    if target_device is not None and row_tensor.device != target_device:
                        row_tensor = row_tensor.to(target_device)
                    row_kwargs[key] = row_tensor
                else:
        ...
        def _cuda_mem_stats():
            ...
            for device_idx in range(torch.cuda.device_count()):
                stats[f"cuda:{device_idx}"] = {
                    "allocated_mb": round(float(torch.cuda.memory_allocated(device_idx)) / (1024 ** 2), 2),
                    "reserved_mb": round(float(torch.cuda.memory_reserved(device_idx)) / (1024 ** 2), 2),
                    "max_allocated_mb": round(float(torch.cuda.max_memory_allocated(device_idx)) / (1024 ** 2), 2),
                }
            return stats

        def _compute_future_candidate_scores(candidate_tokens, model_kwargs_after_outputs, expanded_token_weights, attn_pos):
            zero_column = lambda: torch.zeros(
                (candidate_tokens.shape[0], 1),
                dtype=torch.float32,
                device=candidate_tokens.device,
            )
            future_score_columns = [zero_column() for _ in range(num_attn_candidates)]
            future_visual_columns = [zero_column() for _ in range(num_attn_candidates)]
            future_text_columns = [zero_column() for _ in range(num_attn_candidates)]
            future_ratio_columns = [zero_column() for _ in range(num_attn_candidates)]
            future_failed_columns = [[False for _ in range(candidate_tokens.shape[0])] for _ in range(num_attn_candidates)]
            future_error_columns = [[None for _ in range(candidate_tokens.shape[0])] for _ in range(num_attn_candidates)]

            if not future_enabled:
                return (
                    torch.cat(future_score_columns, 1),
                    torch.cat(future_visual_columns, 1),
                    torch.cat(future_text_columns, 1),
                    torch.cat(future_ratio_columns, 1),
                    future_failed_columns,
            ...
            model_kwargs_after_outputs_cache_devices = None
            if model_kwargs_after_outputs.get("past_key_values") is not None:
                model_kwargs_after_outputs_cache_devices = tuple(
                    tuple(tensor.device for tensor in layer_state)
                    for layer_state in model_kwargs_after_outputs["past_key_values"]
                )
                if any(device.type != "cpu" for layer in model_kwargs_after_outputs_cache_devices for device in layer):
                    model_kwargs_after_outputs["past_key_values"] = tuple(
                        tuple(tensor.cpu() for tensor in layer_state)
                        for layer_state in model_kwargs_after_outputs["past_key_values"]
                    )
            ...
            try:
                for candidate_id in range(min(int(chord_future_topk), num_attn_candidates)):
                    input_ids_tmp = torch.cat([input_ids, candidate_tokens[:, candidate_id].unsqueeze(-1)], dim=-1)
                    model_kwargs_tmp = model_kwargs_after_outputs.copy()
                    ...
                    outputs_tmp = self(
                        **model_inputs_tmp,
                        ...
                        output_attentions=output_attentions,
                        output_hidden_states=output_hidden_states,
                        last_attention_only=output_attentions,
                    )

                    candidate_future_scores = torch.zeros(
                        candidate_tokens.shape[0],
                        dtype=torch.float32,
                        device=candidate_tokens.device,
                    )
                    candidate_future_visual = torch.zeros_like(candidate_future_scores)
                    candidate_future_text = torch.zeros_like(candidate_future_scores)
                    candidate_future_ratio = torch.zeros_like(candidate_future_scores)
                    candidate_future_failed = [False for _ in range(input_ids_tmp.shape[0])]
                    candidate_future_errors = [None for _ in range(input_ids_tmp.shape[0])]
                    future_attn_rows = outputs_tmp.attentions[-1].clone().max(1).values.squeeze(1).to(torch.float32)
                    future_next_tokens = outputs_tmp.logits[:, -1].argmax(dim=-1)
                    future_past_devices = tuple(
                        tuple(tensor.device for tensor in layer_state)
                        for layer_state in outputs_tmp.past_key_values
                    )
                    future_past_key_values = tuple(
                        tuple(tensor.cpu() for tensor in layer_state)
                        for layer_state in outputs_tmp.past_key_values
                    )
                    del outputs_tmp, model_inputs_tmp, model_kwargs_tmp
                    outputs_tmp = None
                    model_inputs_tmp = None
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()

                    for row_idx in range(input_ids_tmp.shape[0]):
                        token_weights_row = None
                        running_prefix = None
                        attn_row = None
                        next_token = None
                        running_model_kwargs = None
                        outputs_roll = None
                        model_inputs_roll = None
                        attn_roll = None
                        future_trace = None
                        failure_stage = "init"
                        failure_mem = None
                        try:
                            failure_stage = "prepare_first_step"
                            failure_mem = _cuda_mem_stats()
                            token_weights_row = expanded_token_weights[row_idx].to(dtype=torch.float32)
                            running_prefix = input_ids_tmp[row_idx].clone()
                            attn_row = future_attn_rows[row_idx]
                            next_token = future_next_tokens[row_idx].view(1).to(running_prefix.device)
                            failure_stage = "build_row_future_cache"
                            failure_mem = _cuda_mem_stats()
                            running_model_kwargs = _slice_model_kwargs_row(
                                model_kwargs_after_outputs,
                                row_idx,
                                target_device=running_prefix.device,
                                include_past=False,
                            )
                            running_model_kwargs["past_key_values"] = _clone_past_key_values_row(
                                future_past_key_values,
                                row_idx,
                                target_devices=future_past_devices,
                            )
                            if "attention_mask" in running_model_kwargs:
                                attention_mask = running_model_kwargs["attention_mask"]
                                running_model_kwargs["attention_mask"] = torch.cat(
                                    [attention_mask, attention_mask.new_ones((attention_mask.shape[0], 1))], dim=-1
                                )
                            if "token_type_ids" in running_model_kwargs:
                                token_type_ids = running_model_kwargs["token_type_ids"]
                                running_model_kwargs["token_type_ids"] = torch.cat(
                                    [token_type_ids, token_type_ids[:, -1].unsqueeze(-1)], dim=-1
                                )

                            def _continuation_step_fn(current_prefix):
                                nonlocal running_model_kwargs, outputs_roll, model_inputs_roll, attn_roll, next_token
                                nonlocal failure_stage, failure_mem
                                rollout_step_idx = max(1, current_prefix.shape[0] - running_prefix.shape[0])
                                failure_stage = f"rollout_step_{rollout_step_idx}"
                                failure_mem = _cuda_mem_stats()
                                model_inputs_roll = self.prepare_inputs_for_generation(
                                    current_prefix.unsqueeze(0), **running_model_kwargs
                                )
                                outputs_roll = self(
                                    **model_inputs_roll,
                                    return_dict=True,
                                ...
                                attn_roll = outputs_roll.attentions[-1][0].clone().max(0).values.squeeze(0).to(torch.float32)
                                next_token = outputs_roll.logits[0, -1].argmax().view(1).to(current_prefix.device)
                                ...
                                del outputs_roll, model_inputs_roll
                                outputs_roll = None
                                model_inputs_roll = None
                                if torch.cuda.is_available():
                                    torch.cuda.empty_cache()
                                return next_token_id, attn_roll

                            future_trace = greedy_future_rollout_from_bootstrap(
                                prefix_ids=running_prefix,
                                bootstrap_next_token_id=int(next_token.item()),
                                bootstrap_attn_last_row=attn_row,
                                ...
                                token_weights=token_weights_row,
                                horizon=int(chord_future_horizon),
                                lambda_txt=float(chord_lambda_txt),
                            )
                            candidate_future_scores[row_idx] = future_trace.result.f_future
                            candidate_future_visual[row_idx] = future_trace.result.sum_visual
                            candidate_future_text[row_idx] = future_trace.result.sum_text
                            candidate_future_ratio[row_idx] = future_trace.result.r_future
                        except Exception as exc:
                            candidate_future_failed[row_idx] = True
                            candidate_future_errors[row_idx] = {
                                "stage": failure_stage,
                                "error": str(exc),
                                "memory": failure_mem,
                            }
                        finally:
                            del token_weights_row, running_prefix, attn_row, next_token
                            del running_model_kwargs, future_trace, attn_roll
                            if outputs_roll is not None:
                                del outputs_roll
                            if model_inputs_roll is not None:
                                del model_inputs_roll
                            if torch.cuda.is_available():
                                torch.cuda.empty_cache()

                    future_score_columns[candidate_id] = candidate_future_scores.unsqueeze(1)
                    future_visual_columns[candidate_id] = candidate_future_visual.unsqueeze(1)
                    future_text_columns[candidate_id] = candidate_future_text.unsqueeze(1)
                    future_ratio_columns[candidate_id] = candidate_future_ratio.unsqueeze(1)
                    future_failed_columns[candidate_id] = candidate_future_failed
                    future_error_columns[candidate_id] = candidate_future_errors
                    del input_ids_tmp, future_attn_rows, future_next_tokens, future_past_key_values, future_past_devices
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
            finally:
                if model_kwargs_after_outputs_cache_devices is not None:
                    model_kwargs_after_outputs["past_key_values"] = tuple(
                        tuple(
                            tensor if tensor.device == target_device else tensor.to(target_device)
                            for tensor, target_device in zip(layer_state, target_devices)
                        )
                        for layer_state, target_devices in zip(
                            model_kwargs_after_outputs["past_key_values"],
                            model_kwargs_after_outputs_cache_devices,
                        )
                    )
        ...
        if chord_diagnostics is not None:
            chord_diagnostics.append({"entered_opera_beam_search": True})
        ...
        while True:
            ...
            next_token_logits = outputs.logits[:, -1, :]
            model_kwargs_after_outputs = self._update_model_kwargs_for_generation(
            ...
            for candidate_id in range(num_attn_candidates):
                ...
                attn_last.append(attn_square.cpu()) # [batch_size * num_beams, 1, q+1, kv+1]

                del outputs_tmp, model_inputs_tmp, model_kwargs_tmp, input_ids_tmp, attn_output
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
            ...
            candidate_token_scores -= penalty_weights * penalty_scores.to(candidate_token_scores.device)
            ...
            if expanded_token_weights is not None and float(chord_lambda_cur) != 0.0:
                ...
                _, v_anchor_scores = apply_current_chord_score(
            ...
            del attn_local, attn_local_scores
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            (
                f_future_scores,
                ...
                candidate_tokens,
                model_kwargs_after_outputs,
                expanded_token_weights,
                attn_pos,
            )
            current_state["candidate_token_scores"] = candidate_token_scores_opera.clone()
            current_state["v_anchor_scores"] = v_anchor_scores.clone() if v_anchor_scores is not None else None
            ...
            try:
                ...
                candidate_token_scores = fuse_chord_scores(
                    opera_scores=candidate_token_scores_opera,
                ...
                if chord_diagnostics is not None:
                    after_order = torch.argsort(candidate_token_scores, dim=-1, descending=True, stable=True)
                    chord_diagnostics.append(
                        {
                            "step_index": int(cur_len),
                            ...
                            "r_future": future_ratio_scores.detach().cpu().tolist(),
            ...
            if not return_dict_in_generate and outputs is not None:
                del outputs
                outputs = None

            del candidate_token_scores, candidate_token_scores_opera, candidate_tokens
            del v_anchor_scores, f_future_scores, future_sum_visual_scores, future_sum_text_scores, future_ratio_scores
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            ...
            model_kwargs = model_kwargs_after_outputs
            if model_kwargs["past_key_values"] is not None:
                model_kwargs["past_key_values"] = self._reorder_cache(model_kwargs["past_key_values"], beam_idx)
            del model_kwargs_after_outputs
