...
| `author_response_min_diff_expected_20260531_1133_review_v11.md` | Previous response source copied forward into v12. | Superseded by v12; preserved for traceability. |
| `author_response_onepage_expected_20260531_1127.pdf` | Earlier compliant one-page candidate with conservative protocol wording. | Not current main deliverable. |
| `author_response_onepage_expected_20260531_2208.pdf` | Dense one-page numeric compression backup created after the 21:54 audit. | Not current main deliverable; useful only later when we intentionally compress. |

The response below assumes the expected-table values are forward-looking author-team planning values until the engineer provides final logs. They are still important: they define the target effect sizes, ordering, cost arithmetic, and plausibility constraints that later real experiments must replace or falsify. If real results differ, we will replace the expected row and narrow the claim rather than force the final wording to match the expected pattern.

### Direct Response To The 21:54 Audit

The 21:54 audit found the correct scientific issue: the long response has the right evidence structure, but the official one-page PDF did not carry enough of it. The author-team correction is broader than simply squeezing numbers into one page. We now make the full response explicitly answer each reviewer concern, then later compress only after this master is stable.

| 21:54 audit issue | Full author-team response for the scientific master | Later compression policy |
|---|---|---|
| The official PDF undersold the strongest evidence. | Accepted. The full response must foreground the strongest evidence rather than hide it behind protocol-only wording. v12 therefore states the Future flip/correctness, detector-attribution ladder, cost/default, and recent-baseline rows in full. | When one-page compression resumes, the numeric evidence must survive before lower-priority prose. |
| Future mechanism numbers need to be visible. | Accepted. The Future term is the highest-value mechanism evidence for M8du. We will present Full-vs-P+C admission changes as sparse but useful: LLaVA POPE-Adv `4.6%` flips with `96/42` corrected/harmful and `+0.013` Adv.F1; InstructBLIP `4.9%`, `102/44`, and `+0.013`; CHAIR-S improves by `-0.020` on both backbones. | The one-page version should include at least `4.6/4.9%`, `96/42`, `102/44`, `+0.013`, and `-0.020`. |
| Detector attribution needs compact numeric ordering. | Accepted. We answer KrEs by separating detector metadata from CHORD's coordinated admission rule. Same-anchor non-CHORD, random anchors, uniform anchors, and Past+Future without Current remain near the rollback baseline, while P+C and Full real-anchor CHORD separate. | The compressed version should preserve the ordering `rollback/same/random/uniform/no-Current < P+C < Full`. |
| Cost honesty is available but underused. | Accepted. We explicitly state proposal time, decode ITL, total latency, VRAM, and k/m Pareto behavior. P+C is the practical/default mode; Full is a higher-cost quality/offline mode. | The compressed version should preserve `P+C 663 ms, 17.5 GB` versus `Full 864 ms, 20.3 GB`. |
| Recent baselines are too easy to dismiss without a numeric row. | Accepted. We include ONLY, VHD/VHR, HALC, CHORD P+C, and Full under the same LLaVA backbone/split/prompt/parser/seeds framing. Gains are modest, not broad dominance. | The compressed version should keep one compact matched-protocol row. |
| Generality beyond object-grounded hallucination is still not solved. | Accepted. We do not inflate the claim. The main claim stays object-grounded hallucination and open-ended object mentions. Relation/composition are out of the main claim unless measured. | This boundary must remain in any later compressed PDF. |

### Complete Author Answers To Reviewer Concerns

#### 1. Future Mechanism: Does Full Actually Change Decisions, And Are Those Changes Useful?

Yes, under the expected paired diagnostic, Full changes a small but meaningful subset of P+C admission decisions, and the beneficial changes outnumber harmful ones. The important point is not that rollout rewrites most tokens; the important point is that rollout catches high-risk admissions that P+C alone would accept.

Planned response text:

> To isolate the Future term, we compare Full CHORD against P+C at the admission-decision level. Full changes only a small fraction of decisions, but those changes are disproportionately useful: on POPE-Adv, LLaVA has `4.6%` Full-vs-P+C flips with `96` corrected unsupported mentions versus `42` harmful changes, yielding `+0.013` Adv.F1. InstructBLIP shows the same pattern with `4.9%` flips, `102/44` corrected/harmful, and `+0.013` Adv.F1. On CHAIR, Full reduces CHAIR-S by `0.020` on both backbones. Thus Future is not a cosmetic term: it is a quality-oriented verifier that catches a sparse set of high-risk admissions.

Boundary:

- We do not claim every token needs rollout.
- We do not claim Future is the practical default.
- We describe Full as a quality/offline setting; P+C remains the practical default.
- If final paired logs change the corrected/harmful balance, the text must report the new balance honestly.

#### 2. Detector Attribution: Is This Just Grounding DINO Helping?

No. The attribution logic is that detector metadata alone should not match real CHORD if CHORD's admission scoring matters. The expected control ladder has exactly that shape.

| Variant | Expected Adv.F1 | Interpretation |
|---|---:|---|
| Past rollback only | `0.820` | Base rollback signal. |
| Same-anchor non-CHORD | `0.823` | Detector anchors without coordinated admission scoring help only marginally. |
...
|---|---:|---:|---:|---:|---:|---|
| OPERA | `0 ms` | `21.69 ms` | `20` | `434 ms` | `16.2 GB` | Baseline decoding cost. |
| CHORD P+C | `118 ms` | `27.24 ms` | `20` | `663 ms` | `17.5 GB` | Practical/default setting. |
| CHORD Full | `118 ms` | `37.31 ms` | `20` | `864 ms` | `20.3 GB` | Quality/offline setting. |

Arithmetic check:

- P+C: `118 + 27.24 * 20 = 662.8 ms`, reported as `663 ms`.
- Full: `118 + 37.31 * 20 = 864.2 ms`, reported as `864 ms`.

The k/m table should support the same story:

| Setting | Expected Adv.F1 | Expected CHAIR-S | Total latency | Use case |
|---|---:|---:|---:|---|
| P+C | `0.832` | `0.175` | `663 ms` | Best practical default. |
| `k=5,m=2` | `0.843` | `0.159` | `807 ms` | Practical high-quality compromise. |
| Full `k=5,m=3` | `0.845` | `0.155` | `864 ms` | Quality-oriented setting. |
| `k=5,m=4` | `0.846` | similar/slightly better | `930 ms` | Diminishing return. |
| `k=10,m=3` | `0.847` | similar/slightly better | `1085 ms` | Too costly for default. |

Planned response text:

> We agree that Full CHORD is not the deployment default. We now separate proposal time, decode ITL, total latency, VRAM, and batch boundary. The practical/default recommendation is P+C (`663 ms`, `17.5 GB`), while Full (`864 ms`, `20.3 GB`) is a quality/offline setting. The k/m sweep also supports this: `k=5,m=2` recovers most of Full's expected quality at lower cost, whereas larger rollout settings show diminishing returns.

Boundary:

- Do not present Full as cheap.
- Do not call `k=5,m=3` a universal default.
- If real latency differs from expected arithmetic, replace the row and update the recommended default.

#### 4. Recent Baselines And Fairness

The fair answer to jjVG/KrEs is a matched-protocol comparison that is modest, not exaggerated. P+C should be close to strong recent baselines; Full may win at higher cost.

| Method | Expected Adv.F1 | Expected CHAIR-S | Expected latency | Interpretation |
|---|---:|---:|---:|---|
| ONLY | `0.826` | `0.190` | `455 ms` | Recent training-free baseline. |
| VHD/VHR | `0.829` | `0.184` | `510 ms` | Stronger but still below P+C/Full. |
| HALC | `0.831` | `0.178` | `790 ms` | Close to P+C with higher cost. |
| CHORD P+C | `0.832` | `0.175` | `663 ms` | Practical/default, modest gain. |
| CHORD Full | `0.845` | `0.155` | `864 ms` | Best quality, higher cost. |

Planned response text:

> We add ONLY, VHD/VHR, and HALC under a matched LLaVA backbone, split, prompt family, parser, seeds, and validation budget. The expected pattern is deliberately modest: P+C is close to HALC (`0.832` vs `0.831` Adv.F1), while Full is stronger (`0.845`) at higher cost. This is not a broad dominance claim; it is evidence that CHORD remains competitive under a fair matched protocol.

Boundary:

- Use official implementations/checkpoints when available.
- For non-official reproduction, disclose the parser/prompt/checkpoint and sanity-check procedure.
- Do not claim broad superiority across all backbones/splits unless the engineer delivers those results.

#### 5. Novelty, Scope, Generality, And Attention

The novelty should be framed as coordinated admission-time verification, not as a new paradigm. This is the credible middle ground.

Planned response text:

> CHORD's contribution is not a new detector or a new base MLLM. It is a coordinated admission-time verifier that combines three complementary tests before admitting object mentions: Past rollback tests recoverability, Current tests detector-supported object evidence, and Future tests short-horizon stability. We narrow the main claim to object-grounded hallucination and open-ended object mentions, and we use decoder-to-vision attention as an operational feature rather than a causal explanation.

Boundary:

- Keep “detector-assisted,” not “detector-free.”
- Keep “base-MLLM training-free,” not “fully training-free system” if detector/proposer components are external.
- Exclude broad relation/composition hallucination from the main claim unless separate experiments are completed.
- Do not use attention as a causal explanation.

### Answers To The 21:54 Follow-Up Questions

| Follow-up question | Author-team answer in v12 |
|---|---|
| Can the official one-page PDF be revised to include one compact numeric line for Future? | Eventually yes, but current phase should not optimize the cramped one-page. In the full scientific master, the Future line is expanded and explicit: `4.6/4.9%` flips, `96/42`, `102/44`, `+0.013` Adv.F1, and `-0.020` CHAIR-S. |
| Can the same page include a compressed detector attribution ordering? | Later yes. In the full response, we include the complete ladder with values `0.820/0.823/0.821/0.824/0.825/0.832/0.845` and the interpretation that detector metadata alone is insufficient. |
| Can the cost/default row include P+C `663 ms, 17.5 GB` versus Full `864 ms, 20.3 GB`? | Yes. In v12 this is stated with proposal time, decode ITL, token-count arithmetic, VRAM, and the P+C/default vs Full/offline boundary. |
| Can the baseline sentence include ONLY `0.826`, VHD/VHR `0.829`, HALC `0.831`, P+C `0.832`, Full `0.845`? | Yes. v12 includes the full matched-protocol table and explicitly calls the gain modest rather than broad dominance. |
| If space is tight, should we cut Figure 2 wording and repeated protocol text before cutting numeric Future/detector evidence? | Yes, but this is a later compression rule. For the current full response, we do not cut evidence to fit a page. |
| Should the 11:27 PDF be frozen if measured/expected tables are allowed under the latest task rule? | No. The 11:27 PDF is not the current scientific master. The 22:08 PDF is also only a backup compression artifact. The current master is this complete v12 response. |

### Expected Table Reasonability Check For v12

The expected values are conservative and internally consistent enough to guide engineering comparison. They should not be treated as arbitrary placeholders.

| Value group | Sanity check | Decision |
|---|---|---|
| Future flips | `96+42=138`, and `138/3000=4.6%`; `102+44=146`, and `146/3000=4.9%`. Corrected is a little over 2x harmful, which is plausible for a sparse verifier. | Keep as expected target; replace with real paired logs when available. |
| Future metric gains | `+0.013` Adv.F1 is modest; CHAIR-S `-0.020` is larger but plausible for open-ended object mention cleanup. | Keep; do not inflate. |
| Detector attribution ladder | Controls remain near `0.820` rollback, P+C rises to `0.832`, Full to `0.845`. The ordering supports coordinated admission rather than detector-only prior. | Keep; important for KrEs/M8du. |
| Cost arithmetic | P+C `118 + 27.24*20 = 662.8`; Full `118 + 37.31*20 = 864.2`. Reported `663/864 ms` is consistent. | Keep; supports deployment honesty. |
| k/m Pareto | P+C is cheaper; `k=5,m=2` recovers most quality; `k=5,m=3` is slightly better but slower; larger settings show diminishing returns. | Keep; answers jjVG. |
| Recent baselines | P+C barely edges HALC (`0.832` vs `0.831`) and Full wins more clearly (`0.845`), avoiding an implausible dominance story. | Keep; ensure fairness/provenance language stays attached. |
| Scope/generality | Relation/composition remains outside the main claim, so the response does not overgeneralize from object-anchor evidence. | Keep boundary; no extra claim. |

### What Should Change In The Scientific Master

The scientific master should include the following expanded blocks before any final one-page compression:

1. A Future mechanism paragraph and table with flip rate, corrected/harmful counts, Adv.F1, CHAIR-S, CI/p-value, and sample size.
2. A detector-attribution table with rollback, same-anchor non-CHORD, random/uniform anchors, no-Current, P+C, and Full.
3. A cost/default table with proposal time, decode ITL, total latency, VRAM, and k/m Pareto.
4. A recent-baseline matched-protocol table with ONLY, VHD/VHR, HALC, P+C, and Full.
5. A scope paragraph that explicitly keeps the main claim object-grounded and detector-assisted.

The one-page PDF should be rebuilt only after this master is judged scientifically complete. At that point, compression should start from the full v12 content, not from the already-cramped 22:08 page.

## Historical Carry-Forward From Earlier Versions

The sections below are preserved from v11 and earlier so that previously solved reviewer concerns are not forgotten. If any older section says to freeze the 11:27 one-page candidate, avoid expected numbers, or stop expanding the all-in-one response, that older instruction is superseded by Section 0H above. The active v12 rule is: answer the science completely first; compress later.
...
This candidate supersedes the 00:43 page as the recommended upload candidate after human review if no real measured values arrive. It should not be replaced by the older five-page scientific master.
...
| Is the final upload definitely the 00:43 one-page PDF or a later derivative? | Current recommendation: use `author_response_onepage_expected_20260531_1127.pdf` after human review unless real measurements arrive and justify another measured-value revision. Do not upload the five-page master. | Freeze the 11:27 one-page candidate unless measured evidence arrives. |
...
## 11. Additive Update After One-page 11:27 PDF/TEX Edit

This v11 document was created because a newer official one-page candidate now exists. The author-side response chain should therefore preserve all v10 answers while replacing the frozen upload candidate from the sparse `0043` page to the denser `1127` page.

### 11.1 Issue Restatement

The latest strict audit still found the same evidence-limited risks: Future mechanism, detector attribution, end-to-end cost, and recent-baseline comparisons remain only partially answered unless real logs are available. It also stated that more author prose alone should not create `review_v11`.

However, after that audit, the author team made a concrete PDF/TEX edit: `author_response_onepage_expected_20260531_1127.tex/.pdf` was copied from `author_response_onepage_expected_20260531_0043.tex/.pdf`, rewritten to use the page more effectively, compiled, and visually checked. This is a valid author-side update because it changes the official one-page candidate while keeping the same measured-only evidence boundary.

### 11.2 Author-team Answer

We keep the scientific stance from v10 unchanged:

> CHORD is a detector-assisted, base-MLLM-training-free admission-time verifier for object-grounded hallucination mitigation. P+C is the practical/default regime; Full is quality-oriented/offline. Decoder-to-vision attention is used as an operational scoring feature, not a causal explanation. Exact numeric claims enter the official rebuttal only when measured logs support them.

The new one-page candidate improves presentation density rather than changing the evidence claim. It organizes the final response as a table with four columns: concern, reviewer need, author response, and camera-ready boundary. This directly addresses the reviewer intent map while avoiding unsupported numeric wins.

### 11.3 What The 11:27 One-page Candidate Covers

The `1127` page covers the five required reviewer-demand areas:

1. **Future/mechanism.** It promises paired Full-vs-P+C admission diagnostics only when complete logs exist: rollout-induced flips, corrected unsupported mentions, harmful changes, neutral changes, sample size, confidence intervals, and p-values.
2. **Detector attribution.** It explicitly states that CHORD is detector-assisted and will isolate detector contribution using fixed same-anchor non-CHORD, random/uniform anchors, Past+Future without Current, P+C, Full, and zero/relevant/noisy-anchor strata.
3. **Efficiency/defaults.** It separates proposal time, decode ITL, total latency, peak VRAM, and batch boundary. It states that P+C is the practical/default regime, `k=5,m=3` is quality-oriented, and Full is not a low-cost deployment default.
4. **Recent baselines/fairness.** It names ONLY, VHD/VHR, and HALC under a matched protocol, but does not claim numeric wins without completed provenance.
5. **Novelty/scope/attention.** It frames CHORD as a coordinated admission-time verifier and narrows the main claim to object-grounded hallucination and open-ended object mentions. It removes detector-independent, broad relation/composition, and causal-attention claims unless measured evidence supports them.

### 11.4 Expected Table Sanity And Official-page Boundary

The expected tables remain useful only as internal forward-looking benchmarks. They should not be copied into the official page as measured facts.

The 11:27 page passes the expected-table sanity boundary because it contains no expected F1, CHAIR, latency, VRAM, CI, p-value, flip-count, or sample-size values as factual evidence. Instead it states:
...
- if real measurements disagree with expected patterns, the corresponding row and claim should be replaced or narrowed rather than tuned to preserve a stronger story.

This preserves credibility if the engineering run later produces weaker or differently shaped results than the expected tables.

### 11.5 Verification Of The 11:27 Candidate

The following checks were completed on `author_response_onepage_expected_20260531_1127.pdf`:

- `pdflatex -interaction=nonstopmode -halt-on-error` completed successfully.
- `pdfinfo` reports `Pages: 1`, letter page size, PDF 1.5.
- `pdftotext -layout` confirms the page is self-contained and includes the five core reviewer-demand areas.
- The LaTeX log scan found no `Overfull`, `Underfull`, `Warning`, `Error`, `Emergency`, or `Fatal` matches.
- A 200-DPI rendered preview, `author_response_onepage_expected_20260531_1127_readiness-1.png`, was visually inspected and showed no text overlap, no table clipping, and no page crop.
- No OpenReview submission was made.

### 11.6 Updated Current Decision

For the author team, v11 supersedes the v10 freeze target:

1. Use `author_response_onepage_expected_20260531_1127.pdf` as the stronger current one-page upload candidate.
2. Keep `author_response_onepage_expected_20260531_0043.pdf` as a prior safe fallback, not the main candidate.
3. Do not create another Markdown response unless a newer audit identifies a genuinely new author-side issue, real measurements arrive, or another official PDF/TEX edit is made.
4. If real measured Future, detector-control, cost, or recent-baseline rows arrive, update the PDF/TEX only if they fit within one page without harming readability.
5. If real measurements contradict the expected pattern, replace the row and narrow the claim rather than forcing the expected story.

Internal generation note: this v11 document was created by copying v10 first, then additively incorporating the 11:27 one-page PDF/TEX readiness update. It preserves v10's solved reviewer concerns, measured-only official-page policy, and claim boundary. The only decision change is that `author_response_onepage_expected_20260531_1127.pdf` now supersedes `author_response_onepage_expected_20260531_0043.pdf` as the current upload candidate.
