# CHORD Author Response, All-in-One Working Draft v5

This document is written from the author-team perspective. It is a complete working response to the remaining reviewer concerns, not a reviewer score sheet. It preserves the already-solved content from v4 and adds the response to the latest audit of v4: the internal response-control document is scientifically strong enough, but the official rebuttal will only be credible if the final strict one-page PDF is built from evidence-eligible content. The core decision is no longer "what more can we say"; it is "which evidence is safe enough to put into the strict one-page official rebuttal, and what exact wording do we use when a row is expected rather than measured."
...
This v5 response is the author-team all-in-one decision document for final-page eligibility. It should guide the next compression step, not encourage further proliferation of expected numeric rows.

Internal generation note: this v5 document was created by copying v4 first, then additively incorporating the latest review_v4 audit. It preserves v4's numeric fixes, evidence-status matrix, one-page eligibility matrix, and final payload rule, then adds a concrete final one-page payload decision, follow-up answers, evidence eligibility sheet, stop rule, and expected-table reasonability check for the latest audit.
