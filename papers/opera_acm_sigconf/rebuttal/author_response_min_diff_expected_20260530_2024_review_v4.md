# CHORD Author Response, All-in-One Working Draft v4

This document is written from the author-team perspective. It is a complete working response to the remaining reviewer concerns, not a reviewer score sheet. It preserves the already-solved content from v3 and adds the response to the latest review of v3: the 17:44 scientific master is now internally coherent, but the remaining risk is final one-page eligibility. The core decision is no longer "what more can we say"; it is "which evidence is safe enough to put into the strict one-page official rebuttal."
...
This v4 response is the author-team all-in-one decision document for final-page eligibility. It should guide the next compression step, not encourage further proliferation of expected numeric rows.

Internal generation note: this v4 document was created by copying v3 first, then additively incorporating the latest review_v3 audit. It preserves v3's numeric fixes and evidence-status matrix, and adds the one-page eligibility matrix and final payload rule.
