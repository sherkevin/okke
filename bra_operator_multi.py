"""
Compatibility shim for the internal, model-coupled BRA/TLRA path.

The adapter abstractions stay here, and the internal intervention mathematics
live in `bra_logits_processor.py`. This module injects that internal processor
into `model.generate(...)`.

Strict universal hot-swappable control should not be routed through these
family-specific hidden-state adapters. That path now belongs to
`bra_universal_plugin.py`, which is designed around decoded strings, frozen
external encoders, and parameter-free tokenizer bridges.
"""
